...
In a previous exercise, we examined an experiment conducted by the popular online auctioneer, eGulf, where a random sample of dedicated used WePhone sellers was drawn. This sample only included eGulf sellers who typically post 10 pictures of their used WePhones. As an experimental treatment, eGulf temporarily allowed some of these sellers to post up to 15 pictures for each auction of their WePhones, and measured whether posting more than 10 pictures effected final WePhone sales prices. In this exercise, eGulf conducted their experiment again. Their sample size seemed sufficiently large (N=350), and the mean sales prices of users in the treatment group seem substantially higher than in the control group, but are they?

*** =instructions
- Follow the code in the workspace to identify and correct for outliers. Create boxplots of the final sales prices for the treatment and control groups, identify and remove the outliers from our sample, and examine the mean difference between the Treatment and Control group.
...
*** =sample_code
```{r}
#Let's naively estimate the average treatment effect on final sales prices with a t.test.
t.test(eGulf$Final_Sales_Price[eGulf$Treatment==1],eGulf$Final_Sales_Price[eGulf$Treatment==0])
#The ATE is almost $36, but the results are not statistically significant. This semms suspicious; usually a large means difference with a sample of this size would be statically significant. 

#Let's examine the distribution of the treatment and control groups with a box plot. Fill in the box plot with the final sales prices for the treatment and control groups, much like how you would for a t.test
Solution1<- boxplot()

#Notice that there are a few dots above each box plot. These dots indicate outliers, specifically, the cutoff for outliers in the boxplot is any value that is 1.5 times the interquartile range above the upper quartile or below the lower quartile. 

#Is it possible that these outliers are driving our treatment effect? Let's subset our data to not include any WePhone sales that were above $1500. This should take care of our extreme outliers.

eGulf<-eGulf[] #Subset eGulf data to exclude final sales prices that were higher than $1500.

Solution2<- #Now calculate the mean difference in final sales prices between the treatment and control group.
...
Solution2<-mean(eGulf$Final_Sales_Price[eGulf$Treatment==1])-mean(eGulf$Final_Sales_Price[eGulf$Treatment==0])
```
*** =sct
```{r}
test_object("Solution1")
