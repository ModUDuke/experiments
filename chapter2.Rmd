...
# The t.test function in R returns quite a lot of results, and it doesn't directly provide us with our treatment effect. But what we are looking for is just the last line: there it shows the two estimated means for the X and Y variables that we inserted into the function. To get the difference between these means, you'll need to do a separate subtraction by hand. It's a little awkward, but at least it's easy! 
...
# Note: Again, the last line of the t-test results show the two estimated means for the X and Y variables that we inserted into the function. To get the difference between these means, you'll need to do a separate subtraction by hand.
```
