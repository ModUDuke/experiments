...
*** =sample_code
```{r}
# Let's first take a look at the structure of the dataframe
  str(WePhones) # Notice that each variable is a factor, with two levels. 

# Let's practice identifying whether the treatment and control are different for Color
  # For ease of computation, let's subset the WePhones dataframe into two new dataframes, one for phones in the treatment, and one for phones in the control
  ...
  # These results show that there is a 0.46 (or 46%) chance that the difference in the means between the number of black phones in the treatment and control groups is equal to 0. In other words, it is fairly likely that the number of black phones in the treatment and control groups are the same. Typically, we only refute the null-hypothesis (in this case, that the phone specifications are imbalanced) when there is less than a 5% chance of it being true. Also, since there are only two different colors in the treatment and control groups, we do not need to test whether they have roughly the same proportion of platinum phones - the results will be identical.  
      
  #Use the t.test function to search through the treatment and control groups of each other specification between the phones until you find one that is imbalanced (i.e. where the probability that the mean difference between the specification in the treatment and control groups i less than 5%)
        
t.test(  )$p.value
      
Solution1<-" " #Write the name of the unbalanced trait between control and treatment groups (precisely)
...
success_msg("Good work! The methods for computing balance in this exercise are very common to experimental research. You may want to return to this exercise and look at how we computed imbalance if you ever get stuck in later ones.")
```
