...
print(Solution2<- )# Further test the validity of the natural experiment by doing a balance check on the pre-treatment level of credit balance. Report the p-value of a t-test.

print(Solution3<- )# Compute the treatment effect of the offer by calculating the average post-treatment balance level for rainy customers versus sunny customers.

print(Solution4<- )# Do the same as above, but for post-treatment default rates

```

*** =solution
```{r}
Solution1 <- mean(CreditCo$opt_in[CreditCo$rainy==1]-CreditCo$opt_in[CreditCo$rainy==0])
Solution2 <- t.test(CreditCo$balance_pre[CreditCo$rainy==1],CreditCo$balance_pre[CreditCo$rainy==0])$p.value
Solution3 <- mean(CreditCo$balance_post[CreditCo$rainy==1]-CreditCo$balance_post[CreditCo$rainy==0])
Solution4 <- mean(CreditCo$default_post[CreditCo$rainy==1]-CreditCo$default_post[CreditCo$rainy==0])
