...
#Graph results
  ...
  bargraph<-barplot(Means,ylim=c(325, 375),col=c(3,5))  
  arrows(x0=bargraph, y0=Means-(1*CIs), y1=Means+(1*CIs), code=3, angle=90,length=.4)
...
*** =sample_code
```{r}
#To check for noncompliance in an experiment, we want to just compare individuals were offered treatment. Therefore, it might be useful to first subset the eGulf data.frame to sellers who were offered treatment. 
...
#Those who opted into the treatment seemed to have slightly higher feedback scores than those who did not, but is the difference statistically significant? Use a t.test and look at the 95% confidence interval. Since we are measuring the mean difference between two values, the confidence interval indicates a range of values for the average treatment effect that we would expect to find if we gathered a new sample. If the lower and upper bound of the 95% confidence interval are both positive, we can conclude that there is a positive and statistically significant difference between the two groups. 

Solution1<- "" #Among those offered treatment, did those who opted into treatment have seller feedback scores that were significantly higher than those who did not opt into treatment? Answer with "Yes" or "No"


Solution2<- # Estimate the correlation between seller feedback scores and final sales prices
```
*** =solution
```{r}
...
Solution1<-ifelse(Significant==T,"Yes","No")
...
Solution1<- # Compute the fraction of people who were offered a higher credit limit

Solution2<- # Of the people who were offered, what fraction opted in?

Solution3<- # Compute a naive average treatment effect of late payment (variable `default_post`), where treatment state is defined by having been offered and opted in to the program
