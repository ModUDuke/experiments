...
# 2) It seems that Donny's travel time is usually less than 10 minutes, but it can last as long as 23 minutes! For additional information, let's also plot what Donny's commute time looks like on a density curve. This is just another way of representing Donny's commute time. Use the plot(density()) command on the dataframe Commute.
...
*** = hint
- The quartile function syntax is quantile(dataframe, .xx), where .xx is the decimal value of the quantile
...
# 2) How long Donny should leave himself if he wants to arrive to work on time at least 50% of the time, with 95% confidence? In other words, given our limited sample, at what time are we 95% confident that Donny could leave and get to work on time half of the time? We can do this by examining the confidence interval on the mean of Donny's commute times; specifically, the upper bound of its 95% confidence interval. For reference, this can be computed with a one sample t-test. The upper bound of a t-test object (i.e. class "htest") can be found in $conf.int[2].
