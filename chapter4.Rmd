...
msg3 = "Even with a well-designed experiment, you still will have just a sample of the possible data to look at, so that's not quite right. Try agai.n"
...
*** = hint
- The quantile function syntax is quantile(variable, .xx), where .xx indicates the quantile of the variable you want to find
- The upper bound of a t-test function can be displayed directly throught the syntax t.test(dataframe)$conf.int[2]


*** =sample_code
```{r}
# 1) We need to determine approximately how much time Donny should leave for his commute if he wants to arrive to work on time approximately 95% of the time. The question did not specify with what confidence, so the answer is simply the commute time that is greater or equal to 95% of the observations in the dataset (i.e. the 95th quantile of commute times). There are several ways to get this answer, but we would recommend using the "quantile" function. The quantile function requires two arguments, the variable name (Commute), and the quantile to be returned (.95). 

#---- Question 1-------------------------------------#
    Solution1<- quantile(var,quantile)
#----------------------------------------------------#

# 2) How long should Donny leave himself if he wants to arrive to work on time at least 50% of the time, with 95% confidence? In other words, given our limited sample, at what time are we 95% confident that Donny could leave and get to work on time half of the time? We can do this by examining the confidence interval on the mean of Donny's commute times; specifically, the upper bound of its 95% confidence interval. The confidence interval of a mean can be determined with the t.test function; the upper bound of an object titled `variable` can be returned with t.test(variable)$conf.int[2].
...
After fishing for a 30 consecutive days in the Atlantic ocean, Edgar, a professional bass fisherman, is asked by his 8 year old daughter, Rachel, why he can't just fish in a nearby lake. To keep his explanation simple, Edgar tells Rachel that the fish in the lake are much smaller than out at sea. Rachel is unconvinced, and tests her father's excuse through an experiment: she spends one day fishing off of a dock on the Atlantic ocean, and one day fishing off of a dock at her nearby lake. Rachel catches many fish during this experiment, but the size of the fish that Rachel caught at the lake were much larger than those caught at the Atlantic. Rachel's experiment is problematic. Which of the following best describes the problem in Rachel's experimental design.
