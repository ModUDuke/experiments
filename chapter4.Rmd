...
#Question 2 asks us to determine how long Donny should leave himself if he wants to arrive to work on time at least 50% of the time, with 95% confidence. In other words, at what time are we 95% confident that Donny could leave and get to work on time half of the time, given our limited sample? We can do this by examining the confidence interval on the mean of Donny's commute times; specifically, the upper bound of its 95% confidence interval. For reference, this can be computed with a one sample t-test. The upper bound of a t-test object (i.e. class "htest") can be found in $conf.int[2].
