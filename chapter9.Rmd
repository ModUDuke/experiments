...
Although it is wholly misleading, one way that unethical scientists justify their arguments that their studies are "as-if" natural experiments is to ignore when the majority of the variables in their datasets that are imbalanced, and instead only report information on the few variables that are balanced in their datasets (i.e. with basically random assignment). That's bad science, but let's give it a try, just one time!

The dataset `KatiePerrie` contains covariate data from a natural experiment about how repeated exposure to music performed by pop star Katie Perrie (measured as owning a Katie Perrie album) is associated with the debilitating psychological disorder called "choreomania." The human rights activists who gathered this data want to publish an article claiming that Katie Perrie's music should be banned since it appears to be harmful to the public's mental health. However, the control and treatment groups in this "natural experiment" are not balanced in most of the key variables, so it's not a valid experiment. 

But let's ignore that and pretend to be bad analysts to help the human rights activists find one variable, any variable, that shows balance so they can incorrectly justify their assumption that this is an as-if natural experiment! Search through the variables in `KatiePerrie` and name one trait where the treatment and control groups are balanced.

*** =instructions
- 1) Use what you have learned about balance and statistical inference and look through each variable in the dataset `KatiePerrie`. 
- 2) Name a single variable that is balanced between the datasets. 
...
*** =sample_code
```{r}
# Below was the t-test used by the otter enthusiast to get the results listed above. Select the text and click on the "Run Code" button to see the results.
...
# 2) Use the Test1 object to assign the upper bound of the confidence interval. For reference, we assigned the lower bound from Test1 for you below.
...
A previous exercise in Chapter 7 using data from CreditCo showed that noncompliance was an issue in the experiment described there (which involved assigning a treatment group to receive an offer in the mail that allowed recipients to increase their credit limit). While investigating potential reasons, you realized that the weather was different for different customers on the day the offers arrived in customers' mailboxes. Specifically, you realized that heavy storms across the country hit half the zipcodes in the treatment group, while the other half of customers experienced sunny weather the day the offer arrived. You realize this variation in the weather may have created a natural experiment where customers in the rainy zones felt gloomy and decided not to take up the offer. while those in the sunny zones felt cheerful and because of their good mood, took up the offer.
...
*** =instructions
- 1) Compute the average difference in take-up rates (among those offered) for rained-out customers vs. sunny customers.
- 2) Report the p-value from a t-test that checks balance in pre-offer credit balances of those whose weather was rainy vs. sunny.
- 3) Compute the treatment effect of the offer on post-offer credit balances, using rain as a natural experiment.
- 4) Compute the treatment effect of the offer on post-offer default, using rain as a natural experiment.
...
# 1) Using a t-test, assess the validity of this natural experiment by comparing the take-up rate of those who were offered the increase and experienced rainy weather (variable "rainy") vs. those who were offered the increase but did not experience rain.


# 2) Further test the validity of the natural experiment by doing a balance check on the pre-treatment level of credit balance. Report the p-value of a t-test. Hint: use the syntax `t.test(x,y)$p.value` to report only the p-value.


# 3) Compute the treatment effect of the offer by calculating the average post-treatment balance level for rainy-day customers versus sunny-day customers (regardless of whether they were offered the balance increase). Use a t-test to report these results.


# 4) Now let's see if the rain had an effect on post-treatment default rates, which is our outcome of interest. Again, use a t-test.
...
t.test(CreditCo$balance_pre[CreditCo$rainy==1],CreditCo$balance_pre[CreditCo$rainy==0])$p.value
t.test(CreditCo$balance_post[CreditCo$rainy==1],CreditCo$balance_post[CreditCo$rainy==0])
Solution1 <- t.test(CreditCo$default_post[CreditCo$rainy==1],CreditCo$default_post[CreditCo$rainy==0])
```
