...
*** =sample_code
```{r}
#Remember, the average treatment effect is simply the mean outcome of the treatment group minus the mean outcome of the control group.
...
*** =sct
```{r}
msg1 = "Good job! This is an example of a heterogeneous average treatment effect - the treament has different effects on men and women. However, when men and women are pooled, this heterogeneity is masked"
...
*** =sample_code
```{r}
#Since we are interested in understanding the rates of intention to leave by treatment and gender, we might want to consider making a three way table of the results first. The following prints the numbers of men and women who intend to leave work by treatment group. 
  xtabs(~LeaveJob+Treatment+Female, data=UnterHR)
#Since the treatment and control groups among women are of relatively similar sizes, it is fairly easy to get a sense of how treatment affects women's intention to leave Unter; the majority of women in the treatment group appear to want to leave, whereas the majority of women in the control group do not. However, let's actually compute the differences in the Average treatment effects among men and women.
...
The economist from the previous question runs a second experiment, this time measuring the effect of *work hours* on happiness via income as a proxy variable. He helps respondents in his treatment group negotiate to work an additional 10 hours at their primary jobs per week for one year. 

This time he finds a statistically significant effect: increasing work hours appears to increase happiness (measured as income). But the relationship between work hours and happiness might be suspect. A more experienced economist replicates the first economist's study, but also directly measures the effect of work hours on happiness to test whether the original proxy variable was appropriate. 
...
```{r}
str(econometrics)
Solution1<- # Use the 'cor' function to determine and report whether Happiness is correlated with Income
Solution2<- # Use a t.test to determine whether Treatment effects Income, and report its p.value
Solution3<- # Use a t.test to determine whether Treatment effects Happiness, and report its p.value
```
*** =solution
```{r}
...
Let's say that you are writing up a report on an experiment, and your mentor asked you to report all results to the nearest 100th digit. If the p value for your key statistic were 0.0542, and your field tends to consider any result with p-values of 0.05 or less to be statistically significant and valid, what would be the most informative/least misleading number to write down as the p-value in your report?
*** =instructions
- 0.05
- 0.054
- 0.06
...
msg2 = "This is accurate, but this response is not to the nearest 100th digit. Try again."
...
msg4 = "This is accurate, but this response is not to the nearest 100th digit. Try again."
test_mc(correct = 3, feedback_msgs = c(msg1,msg2,msg3,msg4))
```
...
--- type:MultipleChoiceExercise lang:r xp:50 skills:1 key:983026d374
...
The researchers divide a population of preschool-aged children into a treatment group and a control group. Importantly, the children in each of the groups come from a variety of income backgrounds.

35 years after the study, the researchers produce the following table, which documents the Conditional Average Treatment effect for each group of students:
...
A government official asks for your opinion about the study, based solely on these results. What would you tell her? (Note: There is more than one correct answer!)
...
*** =sct
```{r}
msg1 = "Correct! Focusing the policy among the group that gains the most is a sound way to conduct policy."
msg2 = "Correct! Because the gains from preschool are highest among the lower-income group, a government that is interested in redistributing income could focus on providing preschool to low-incom children."
msg3 = "Correct! The results for this particular experiment seem to point to large gains for all groups of children."
msg4 = "This question is tricky, but there is more than one correct answer. Read the answers carefully and try again."
test_mc(correct = c(1,2,3), feedback_msgs = c(msg1,msg2,msg3,msg4))
```
...
A recent study tried to find out who loves their owners more: cats or dogs. The study had a random sample of cat and dog owners play with their pets for 10 minutes, and measured changes in the pets' levels of oxytocin, often referred to as the "love hormone." 
