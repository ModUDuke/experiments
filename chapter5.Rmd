...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:50 skills:1 key:060acdb7c4
## Oregon Health Experiment Data: Data Structures
Let's say you're wondering how much mens' and womens' health differed in the experiment, and what the data says about Medicaid's effects on those differences, so you decide to look at some of the basic health numbers in the data. A simulated version of the Oregon Health Insurance Experiment data, `OHIE`, is available in the workspace. With this dataframe, wee will make some quick calculations to learn about any differences. The first step is to separate out the treatment and control groups into new dataframes.

*** =instructions
- 1) Examine the structure of the `OHIE` dataframe.
- 2) Use the `subset` function to create a new dataframe called `TreatmentGroup` that contains only those selected to be able to receive Medicaid in the treatment group. 
- 3) Use the `subset` function to create a new dataframe called `ControlGroup` that contains only those not selected for Medicaid in the control group.
...
library(dplyr)
set.seed(1)
load(url('http://s3.amazonaws.com/assets.datacamp.com/staging/course_1145/datasets/OHIEexperimental.Rda'))
...
# "http://s3.amazonaws.com/assets.datacamp.com/staging/course_1145/datasets/iris.csv",
...
*** =sample_code
```{r}
# 1) Since the OHIE dataset is fairly big, let's start to get a sense of it by looking at its structure. Use the str() command on the dataframe OHIE:
#---- Question 1a------------------------------------#
    Solution1a<-""  
#----------------------------------------------------#

# 2) Since we are going to run several t-tests, let's make the syntax below simpler by separating the OHIE dataset into treatment and control groups. Based on whether the treatment variable is a 1, subset the OHIE dataframe into a dataframes called TreatmentGroup.
#---- Question 1b------------------------------------#
    Solution1b<-""  
#----------------------------------------------------#   
    
# 3) Now, based on whether the treatment variable is a 0, subset the OHIE dataframe into a dataframes called ControlGroup.
#---- Question 1c------------------------------------#
    Solution1c<-""  
#----------------------------------------------------#    
```
...
```{r}
solution1a<- str(OHIE)
solution1b<- TreatmentGroup <- OHIE[OHIE$treatment==1, ]
solution1c<- ControlGroup <- OHIE[OHIE$treatment==0, ]
```
...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:50 skills:1 key:e3d1a5a3ed
...
A side note: the `mu` value in a t-test is the value of our null hypothesis (i.e. that treatment does nothing), so let's keep that set at 0.
...
- Your answer for the first question should look something like t.test(TreatmentGroup$variable,ControlGroup$variable,mu=0). 
...
*** =sample_code
```{r}
# 1) Since we are still early in the course, we will help you answer the first question. We need to determine whether gender is balanced between the treatment and control groups. This requires a two sample t-test. In R, the format of the t.test function is t.test(Dataframe1$variable, Dataframe2$variable, mu=0). Try this with the treatment and control groups, and our gender variable gender_inp.
#---- Question 2a------------------------------------#
    Solution2a<-t.test(TreatmentGroup$gender_inp,ControlGroup$gender_inp,mu=0)
#----------------------------------------------------#

# 2) To determine whether the t-test shows that gender is balanced, we need to determine whether its results are statistically significant. If the 95% confidence interval includes 0, the results are not statistically significant (i.e. we cannot be confident that there is a difference between the treatment and control group). Based on the confidence interval in Solution 1a, do the results appear statistically significant? Answer with "Yes" or "No." 
#---- Question 2b------------------------------------#
    Solution2b<-""  
#----------------------------------------------------#
```
...
test_error()
success_msg("Good work! It looks like our treatment and control groups are balanced on gender. Let's   We'll find out next")
```



--- type:NormalExercise lang:r aspect_ratio:62.5 xp:50 skills:1 key:1729739153
...
Now let's check the treatment effect on the systolic blood pressure values across our treatment and control groups. To check for statistical significance, let's look at p values of the t-test, which is a more direct way than confidence values. Adding $p.value to the end of the t.test function will return that for us. For example, for solution 2a in the previous question, we could have just typed: "Solution2a <- t.test(TreatmentGroup$gender_inp, ControlGroup$gender_inp, mu=0)$p.value".

Let's try this with our systolic blood pressure variables, bp_sar_inp. 

*** =instructions
- 1) Calculate the average treatment effect on systolic blood pressure with the `mean` command.
- 2) Determine if the treatment and control groups have significantly different average values for systolic blood pressure (variable `bp_sar_inp`).
...
*** =hint
- For each of the following questions, use the `t.test` function where (mu=0) to statistically test whether the treatment and control group are different from each other.
- Your answers for the t-tests should look something like t.test(TreatmentGroup$variable,ControlGroup$variable,mu=0)$p.value. 
...
#  1) Let's calculate the average treatment effect of Medicaid on the systolic blood pressure of units in the experiment.
#---- Question 3a------------------------------------#
    Solution3a<-  
#----------------------------------------------------#

#  2) For Question 3, use a t-test to determine whether the treatment had a statistically significant effect on systolic blood pressure, and return the p-value.
#---- Question 3b------------------------------------#
    Solution3b<-  
#----------------------------------------------------#

# 3) A p.value of less than .05 is generally considered statistically significant. Based on your results in Solution 3a, did the experiment yield a statistically signficant average treatment effect on systolic blood pressure? Answer with "Yes" or "No."
#---- Question 3c------------------------------------#
    Solution3c<-""  
#----------------------------------------------------#
...
*** =solution
```{r}
Solution3a<- mean(TreatmentGroup$bp_sar_inp)-mean(ControlGroup$bp_sar_inp)
Solution3b<- t.test(TreatmentGroup$bp_sar_inp,ControlGroup$bp_sar_inp,mu=0)$p.value
Solution3c<-"No"
```
...
test_object("Solution3c")
test_function("mean", incorrect_msg = "Did you use the `mean` function?")
test_function("t.test", incorrect_msg = "Did you use the `t.test` function?")
test_error()
success_msg("Good work, although that high p-value means that the estimate is not statistically significant. That's not what we were hoping for, but let's see if that's also true of the treatment effect on diastolic blood pressure.")
```


--- type:NormalExercise lang:r aspect_ratio:62.5 xp:50 skills:1 key:3e2623b84b
## Oregon Health Experiment Data: Finding Another ATE
Now let's check the treatment effect on diastolic blood pressure values across our treatment and control groups. Once again, we'll use the values of the t-test to measure statistical significance. As a reminder, adding $p.value to the end of the t.test function will return that for us. For example, for solution 3b in the previous question was "t.test(TreatmentGroup$bp_sar_inp,ControlGroup$bp_sar_inp,mu=0)$p.value"

Let's try this with our second blood pressure variable, bp_var_inp. 

*** =instructions
- 1) Calculate the average treatment effect on diastolic blood pressure.
- 2) Determine if the treatment and control groups have significantly different average values for diastolic blood pressure (variable `bp_dar_inp`).
- 3) After looking at the p value of the t-test, can we say the t-test results are statistically significant? Answer "yes" or "no".
...
*** =sample_code
```{r}
#  1) Let's calculate the average treatment effect of Medicaid on the diastolic blood pressure of units in the experiment.
#---- Question 4a------------------------------------#
    Solution4a<-  
#----------------------------------------------------#

# 2) Return the p-value of a t-test that measures the average treatment effect on diastolic blood pressure.
#---- Question 4b------------------------------------#
    Solution4b<-   
#----------------------------------------------------#

# 3) Based on your t-test, did the treatment have a statistically significant effect on diastolic blood pressure? Answer with "Yes" or "No."
#---- Question 4c------------------------------------#
    Solution4c<-""   
#----------------------------------------------------#
...
*** =solution
```{r}
Solution3d<- mean(TreatmentGroup$bp_dar_inp)-mean(ControlGroup$bp_dar_inp)
Solution3e<- t.test(TreatmentGroup$bp_dar_inp,ControlGroup$bp_dar_inp,mu=0)$p.value
Solution3f<-"Yes"
```

*** =sct
```{r}
test_object("Solution4a")
test_object("Solution4b")
test_object("Solution4c")
test_function("mean", incorrect_msg = "Did you use the `mean` function?")
test_function("t.test", incorrect_msg = "Did you use the `t.test` function?")
test_error()
success_msg("Good work. Our estimate of for the treatment effect on diastolic blood pressure had a p value under .05, so we will say it's statistically significant. Because some of our estimates were not statistically significant, and some are, we probably will need to work a little harder to argue for the validity of our results and interpretations on the effect of Medicaid on health results.")
```
...
*** =instructions
- 1) Determine the difference in means betwen the `Final_Sales_Price` of sellers who posted more than 10 pictures (`Seller_Opted_Into_Treatment`==1) on their auctions, and those who had not.
- 2) Statistically test whether sellers who posted more than 10 pictures (`Seller_Opted_Into_Treatment`==1) on their WePhone auctions (`Final_Sales_Price`) sold their WePhones at higher prices than sellers who posted 10 or fewer pictures on their WePhone auctions (`Seller_Opted_Into_Treatment`==0)
...
*** =sample_code
```{r}
# 1) To begin, let's examine the structure of the dataframe `eGulf` with the str function. 
#---- Question 1-------------------------------------#
    Solution1<-   
#----------------------------------------------------#

# To get an even better sense of the data, let's visualize the data graphically. Boxplots are often ideal for studying data in controlled experiments, as they offer a convenient means to compare two groups (the treatment and control). Select the code in the line below and hit the "Run Code" button.
...
```{r}
Solution1<-str(eGulf)
Solution2<-mean(eGulf$Final_Sales_Price[eGulf$Seller_Opted_Into_Treatment==1])-mean(eGulf$Final_Sales_Price[eGulf$Seller_Opted_Into_Treatment==0])
Solution3<-t.test(eGulf$Final_Sales_Price[eGulf$Seller_Opted_Into_Treatment==1],eGulf$Final_Sales_Price[eGulf$Seller_Opted_Into_Treatment==0])$p.value
