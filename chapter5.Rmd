...
*** =sample_code
```{r}
# Let's print the results from the OregonHealthResults dataframe
OregonHealthResults 

# Now let's practice reading the table identify the following:

Solution1<- # Write the mean value of positive screening in the control group (do not round)

Solution2<- # Write the average treatment effect of medicaid coverage on positive screening

Solution3<-  # Use solution1 and Solution2 to calculate the mean value of positive screening in the treatment group
...
Solution1<-  # Write the 95% confidence interval lower bound for the treatment effect of medicaid coverage on current use Of medication

Solution2<-"" # Write name of variable whose average treatment effect was not statistically significant (must be a string)
...
An easy way to identify whether the variability of a dependent variable is unequal across the range of values in an independent variable is to examine these relationships in a scatter plot. The R Workspace contains the syntax to produce plots for income's relationship with three different predictors: Education, Age, and City Population. Each dot represents an observation from our sample. Using these graphs, determine which independant variable has the most heteroskedastic relationship with income
...
# To determine whether the t.test shows that gender is balanced, we need to determine whether its results are statistically significant. If the 95% confidence interval includes 0, the results are not statistically 0 (i.e. we cannot be confident that there is a difference between the treatment and control group). Based on the confidence interval in Solution1a, do the results appear statistically significant?
  Solution1b<-"" # Is gender balanced? Answer with "Yes" or "No" 

# A more direct way to test for statistical significance is to just look at the p.vale of the t.test. To directly compute the p-value for solution1a, we could have just typed: "Solution1a<-t.test(TreatmentGroup$gender_inp,ControlGroup$gender_inp,mu=0)$p.value". Try to use a t.test to determine whether the treatment had a statistically significant effect on systolic blood pressure, and return the p.value in solution 2.
  Solution2a<-  # Return the p.value of a t.test measuring the average treatment effect on systolic blood pressure 

# A p.value of less than .05 is generally considered statistically significant. Based on your results in Solution2a, did the experiment yield a statistically signficant average treatment effect on systolic blood pressure?  
  Solution2b<-"" # Was there a statistically significant average treatment effect on systolic blood pressure? Answer with "Yes" or "No" 

# Based on the above, try the last two questions by yourself.
  Solution3a<-  # Return the p.value of a t.test that measures the average treatment effect on diastolic blood pressure 

  Solution3b<-""  # Based on your t.test, does the treatment have a statistically significant effect on diastolic blood pressure? Answer with "Yes" or "No" 
...
#Before doing statistical tests, it can often be useful to visualize the data graphically. For controlled experiments, boxplots are often useful. The following boxplot shows the relationship between opting into treatment and final sales prices. The middle line represents the median of the values, and the two other lines that complete the "box" represent the upper and lower quartiles of data in the dataset. The "whiskers" at the edge of the plot represent the most extreme datapoints that are not outliers.
...
Solution1<- #Determine the means difference between the final sales prices of eGulf sellers by treatment condition

#It seems that sellers in the treatment group sold their Wephones at higher prices. Let's conduct a statistical test to make sure whether this finding is robust. 
  
Solution2<- # Report the p.value of a t.test that tests whether opting into treatment (i.e. posting more than 10 pictures) increased final sales prices
```
*** =solution
```{r}
