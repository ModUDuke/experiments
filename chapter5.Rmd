...
sign(Solution1$conf.int[1])==sign(Solution1$conf.int[2])
Solution2<-"No"
```
...
*** =sample_code
```{r}
#  1) Calculate the average treatment effect of Medicaid on the systolic blood pressure of units in the experiment. This is equal to the `mean` value for systolic blood pressure in the treatment group minus the `mean` value for systolic blood pressure in the control group. We help by filling out the front end of this equation
...
#We can computationally determine whether the p-value is lower than .05 with the syntax below
Solution2<-ifelse(Solution2$p.value<.05,"Yes","No")
```
