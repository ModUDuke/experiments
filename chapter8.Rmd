...
# 3) The average treatment effect is simply the mean outcome of the treatment group minus the mean outcome of the control group.
...
*** =solution
```{r}
Solution1<-mean(UnterHR$LeaveJob[UnterHR$Treatment==1])
Solution2<-mean(UnterHR$LeaveJob[UnterHR$Treatment==0])
...
*** =sample_code
```{r}
# Since we are interested in understanding the rates of intention to leave by treatment and gender, we might want to examine a three way table of this relationship first. The following syntax prints the numbers of men and women who intend to leave work by treatment group and gender. Run this code by selecting it with the cursor and clicking the "Run Code" button, or by typing it into the console:
    xtabs(~LeaveJob+Treatment+Female, data=UnterHR)

# So let's look at the balance in raw numbers of units between the treatment and control groups. Among men, there are 214 (141+73) in the control group and 122 (98+24) in the treatment group.  Among women, there are 27 (19+8) in the control group, and 19 (5+14) in the treatment group.  We have a lot more men than women in our samples. 
# So how many men and women want to stay, and how many want to leave? Among men in both treatment and control groups, 239 (141+98) say they want to stay at Unter and 97 (23+74) say they plan to leave. Among women in both groups, 24 (19+5) say they plan to stay, and another 24 (6+18) say that they plan to leave.

# Since the numbers in the leave and and stay groups among women are equal, it is fairly easy to get a sense of how treatment affects women's intention to leave Unter; the majority of women in the treatment group appear to want to leave, whereas the majority of women in the control group do not. But to be sure, let's actually calculate the differences in the average treatment effects among men and women.

# 1)Determine the average treatment effect among men. How big is it? Is it positive or negative?
#---- Question 1-------------------------------------#
    Solution1<- 
#----------------------------------------------------#
      
# How big is it? Is it positive or negative?      

# 2) Determine the average treatment effect among women.
#---- Question 2-------------------------------------#
    Solution2<- 
#----------------------------------------------------#
...
- 3) Demonstrate that there is a statistically significant effect of the treatment, `Treatment`, with the proxy variable, `Income`.
- 4) Determine whether there is a statistically significant effect of the treatment, `Treatment`, with the true outcome of interest, `Happiness`.
...
# 3) Use a t-test to determine whether Treatment affects Income, and report its p-value.
#---- Question 2-------------------------------------#
    Solution3<- 
#----------------------------------------------------#

# 4) Question 3: Use a t-test to determine whether Treatment effects Happiness, and report its p-value.
#---- Question 3-------------------------------------#
    Solution3<- 
#----------------------------------------------------#
...
*** =solution
```{r}
Solution1<-str(econometrics)
Solution2<-cor(econometrics$Happiness,econometrics$Income)
Solution3<-t.test(econometrics$Income[econometrics$Treatment==1],econometrics$Income[econometrics$Treatment==0])$p.value
Solution4<-t.test(econometrics$Happiness[econometrics$Treatment==1],econometrics$Happiness[econometrics$Treatment==0])$p.value
```
...
Suppose that a study similar to the Perry Preschool was interested in how preschool affects future earnings, and in particular how the effects are different by income level of the preschoolers' families. (Recall that Perry Preschool was administered to a heavily low-income population, so this study is slightly different.)
...
*** =hint
- Remember that you are trying to pick the wrong answer -- the thing you would NOT tell the official who asked for your opinion. Do your best!

*** =sct
```{r}
msg1 = "No, focusing the policy among the group that gains the most in absolute dollar terms is a reasonable policy recommendation."
msg2 = "Incorrect. Because the gains from preschool are much higher as a percentage among the lower-income group than the high income group, a government that is interested in redistributing income or concerned about ensuring a minimum standard of living could focus on providing preschool to low-income children."
...
msg4 = "Correct! There **is** enough information to provide an opinion here, so not offering one is not a reasonable response."
test_mc(correct = 4, feedback_msgs = c(msg1,msg2,msg3,msg4))
```
