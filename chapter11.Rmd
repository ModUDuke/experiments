...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:100 skills:2 key:20e9735e18
...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:100 skills:2 key:ac094f96a3
...
- 4) Create a new chart to see if Step 7 helps make our values look more normal.
...
# 2) Since the dataset is relatively small, let's use the `head` and `order` commands to examine the highest DistanceWalked observations in the dataset. The following syntax will order the top results by the outcome of interest, so use it as a guide: head(Dataframe[order(Dataframe$Variable,decreasing=T),])

#----------------------#
     Solution2<- 
#-----------------------#

# 3) Notice that the four largest values for DistanceWalked are more than three times as large as the 5th largest value for DistanceWalked. These are clearly outliers. However, we should also note that all of these values are in the control group, so removing them from our dataset, as we had done with previous outliers, might also bias our estimates by lowering the mean values for the treatment group. Instead, let's say that our maximum distance we will allow is 2. This is known as top coding, and we'll top code these values so that they are still the largest values in the dataset, but so that they are not outliers and do not violate our assumptions of a normal distribution. 
  
# Top coding can be arbitrary (and there are better methods for handling outliers), but for this assignment, use the following syntax as a model to create an ifelse statement that says that any value of DistanceWalked over 2 will be changed to equal 2: dataframe$variable<-ifelse(dataframe$variable>x,y,dataframe$variable). Note: x is the current value of the variable, and y is the final value we want.

#----------------------#
     KittyCatch$DistanceWalked<-
#-----------------------#
    
# 4) If you completed question 7 correctly, the distribution of DistanceWalked should now look much more normal. Create a new density plot of DistanceWalked to see if that is true.
...
*** =solution
```{r}
  Solution1<- plot(density(KittyCatch$DistanceWalked))
  Solution2<- head(KittyCatch[order(KittyCatch$DistanceWalked,decreasing=T),])
  KittyCatch$DistanceWalked<-ifelse(KittyCatch$DistanceWalked>2,2,KittyCatch$DistanceWalked)
  Solution4<- plot(density(KittyCatch$DistanceWalked))
```
*** =sct
```{r}
test_object("Solution1")
...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:100 skills:2 key:9a3e134b3c
...
# 2) It could be problematic if the ages among the treatment group are different than those in the control group, especially if Age is correlated with DistanceWalked. Run a cor() function between Age and DistanceWalked to see if they are indeed correlated. The syntax for this will be cor(dataframe$variable1, dataframe$variable2):

#----------------------#
     Solution2<- 
#-----------------------#

# 3) Are the treatment and control groups balanced on Age, or are they imbalanced? Let's run a t-test to see.  The syntax for this will be: t.test(dataframe$variable[dataframe$Treatment==0],dataframe$variable[dataframe$Treatment==1])
...
```{r}
  ...
  Solution1<- boxplot(KittyCatch$Age[KittyCatch$Treatment==0],KittyCatch$Age[KittyCatch$Treatment==1],names=c("Control","Treatment"))
  Solution2<-cor(KittyCatch$Age,KittyCatch$DistanceWalked)
  Solution3<- t.test(KittyCatch$Age[KittyCatch$Treatment==0],KittyCatch$Age[KittyCatch$Treatment==1])
  Solution5<- t.test(KittyCatch$Age[KittyCatch$Treatment==0],KittyCatch$Age[KittyCatch$Treatment==1])
...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:100 skills:2 key:b2aadb37f4
...
# 3) Even more worrisome, the NA values for TotalHoursPlayed appear at higher rates for respondents in the treatment group than in the control group. Check the average values ofTotalHoursPlayed in both treatment and control groups, in separate lines, using the mean(is.na(dataframe$variable)) function. Remember that you can select units within a dataframe by their Treatment value with the syntax variable[dataframe$Treatment==x]. 
...
```{r}
  ...
  Solution1<- summary(KittyCatch$TotalHoursPlayed)
  Solution2<- cor(KittyCatch$TotalHoursPlayed,KittyCatch$DistanceWalked,use="complete")
  Solution3<-     mean(is.na(KittyCatch$TotalHoursPlayed[KittyCatch$Treatment==1]))
    mean(is.na(KittyCatch$TotalHoursPlayed[KittyCatch$Treatment==0]))
  Solution4<-  t.test(KittyCatch$TotalHoursPlayed[KittyCatch$Treatment==1],KittyCatch$TotalHoursPlayed[KittyCatch$Treatment==0])
...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:100 skills:2 key:9ceed1a402
...
*** =instructions
- 1) Calculate the average treatment effect.
- 2) Check the statistical significance of that result.
...
# 1) Now that we have balanced our data on all known dimensions, let's return the average treatment effect of walking farther on time spent playing KittyCatch by subtracting the means of our control group from our treatment group:
...
# 2) Finally, let's test if the ATE is significant by finding out if the p value of a t-test is less than .05, which we will use as our standard of significance:
...
```{r}
  ...
  Solution2<-t.test(KittyCatch$DistanceWalked[KittyCatch$Treatment==1],KittyCatch$DistanceWalked[KittyCatch$Treatment==0])$p.value
```
*** =sct
```{r}
