...
--- type:NormalExercise lang:r aspect_ratio:62.5 xp:100 skills:2
## Putting it All Together
Often times, our experiments don't always go as planned. This question will test what you have learned to resolve a question with realistic problems.
...
*** =instructions
- Balance the data
- Deterimine the average treatment effect
...
#Notice that the maximum value is much larger than the mean, median, and 3rd quartile of the dataset. This outlier might bias our t.test results by violating its assumption that the data is normally distributed. See density plot below to observe the wrong right tail in the distribution of DistanceWalked.
...
#Notice that the medians and interquartile range are very similar between the treatment and control groups, but that the largest values in the treatment group rest much higher than in the control group. It turns out that this did not happen by chance; by default, KittyCatch only requires respondents to enter their names if they state that they are younger than 18; however, for the experiment, all respondents in the treatment group were required to enter their ages. It could be problematic if the ages among the treatment group are different than those in the control group, especially if Age is correlated with DistanceWalked, which, as demonstrated below, it is.
...
#If you completed question 2 correctly, Age among the treatment and control groups should be balanced
  t.test(KittyCatch$Age[KittyCatch$Treatment==0],KittyCatch$Age[KittyCatch$Treatment==1])
...
#Notice that 96 values are labeled "NA". These cases are missing information, in large part because KittyCatch allows users to opt out of recording how many hours they play. However, as shown below, TotalHours played appears to be correlated with DistanceWalked.
...
#Now that we have balanced our data on all known dimension, let's return the average treatment effect, and test whether it is significant
